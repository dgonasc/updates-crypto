...
const CryptoList: React.FC =  () => {
    ...
    filteredData.sort((a: { lastPrice: number }, b: { lastPrice: number }) =>  b.lastPrice - a.lastPrice);
                    ...
                    {filteredData.map((symbols: any) => {
                        ...
                        return (
                            <li className={"flex items-center justify-between border-2 rounded-3xl p-2 sm:px-9 shadow-lg border-white shadow-white" + color} key={symbols.symbol}>
                                <h2 className="font-serif text-2xl font-semibold tracking-widest text-gray-100 ">{name}</h2>
                                <div className="grid justify-center">
                                    <h3 className="my-2 font-serif text-2xl font-semibold tracking-widest text-gray-100">$ {parseFloat(symbols.lastPrice)}</h3>
